...
export default function Home() {
    ...
    const { user, login, logout, isAuthenticated, liffReady } = useAuth();
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 to-green-50">
                    ...
                    <div className="flex justify-end mb-4">
                        {isAuthenticated ? (
                            ...
                            <button
                                ...
                                disabled={!liffReady}
                                className="bg-white/20 hover:bg-white/30 px-6 py-2 rounded-full font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                {liffReady ? '登入' : '載入中...'}
                            </button>
                        )}
            ...
            {/* Floating Action Button - Story A 入口 (Upload) & Story B 入口 (Request) */}
            <div className="fixed bottom-8 right-8 flex flex-col gap-4 z-50">
                <Link
                    to="/waitlist/new"
                    className="bg-gradient-to-r from-orange-500 to-orange-600 text-white p-4 rounded-full shadow-lg hover:shadow-xl hover:scale-110 transition-all duration-300 flex items-center justify-center"
                    title="新增需求"
                >
                ...
                <Link
                    to="/upload"
                    className="bg-gradient-to-r from-green-500 to-green-600 text-white p-6 rounded-full shadow-2xl hover:shadow-3xl hover:scale-110 transition-all duration-300"
                    title="上傳制服"
                >
